...
import { CURSOR_COMPOSER_FAST_COST, CURSOR_COMPOSER_STANDARD_COST } from "../src/cursor-model-costs.js";
...
describe("cursor usage accounting", () => {
	...
	it("estimates Composer slow alias usage cost from standard API rates", () => {
		const model = makeModel("composer-2.5:slow");
		const context: Context = {
			systemPrompt: "Be helpful.",
			messages: [{ role: "user", content: "Hello", timestamp: 1 }],
		};
		const partial = makeAssistantMessage([{ type: "text", text: "Hello back." }]);
		partial.model = "composer-2.5:slow";

		applyCursorApproximateUsage(partial, model, context, 1000);

		const expectedInput = (CURSOR_COMPOSER_STANDARD_COST.input / 1_000_000) * partial.usage.input;
		const expectedOutput = (CURSOR_COMPOSER_STANDARD_COST.output / 1_000_000) * partial.usage.output;
		expect(partial.usage.cost.input).toBeCloseTo(expectedInput, 8);
		expect(partial.usage.cost.output).toBeCloseTo(expectedOutput, 8);
		expect(partial.usage.cost.total).toBeCloseTo(expectedInput + expectedOutput, 8);
	});
